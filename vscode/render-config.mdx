...
```
WITHOUT render config:

  preview (VSCode)                 production (your app)
  ────────────────                 ─────────────────────
  extension defaults               hand-written params
   └─ pageSize: A4                  └─ pageSize: "A4"
   └─ margin: (none)                └─ margin: {top:"20mm"}
...
WITH render config:
  ...
  preview (VSCode) ─────────┐
                            ├── rendio.json + *.rendio.json
  production (your app) ────┘   (single source of truth)
...
Anything valid on `POST /v1/render` that isn't `html`, `source`, `data`,
...
- [VSCode overview](/vscode/overview) — extension install and features.
- [POST /v1/render](/api-reference/render) — the full parameter list you can put in config files.
